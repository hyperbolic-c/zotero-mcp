...
class LegacyMetadataRetriever(BaseRetriever):
    ...
    def ingest_data(
        ...
        if self._ingest_fn is None:
            return {"error": "Ingest service is not available for legacy_metadata retriever"}
        ...
        **kwargs: Any,
        ...
        if self._search_fn is None:
            return {
                ...
                "results": [],
                "total_found": 0,
                "error": "Search service is not available for legacy_metadata retriever",
            }
        return self._search_fn(query=query, limit=limit, filters=filters)

    def get_database_status(self) -> dict[str, Any]:
        if self._status_fn is None:
            return {"collection_info": {"error": "Status service is not available for legacy_metadata retriever"}}
